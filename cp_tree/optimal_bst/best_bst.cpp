...
void recur(int n,CP::map_bst<int,int> &bst,int curr,int midlength) {
  if (midlength < 2) return;
  int m = midlength/2;
  int left = curr - m, right = curr + m;
  bst[curr] = 1;
  if(left < 1 || right > n) return;
  bst[left] = 1; bst[right] = 1;
  recur(n,bst,left,m);
  recur(n,bst,right,m);
}
void gen_best_bst(int n,CP::map_bst<int,int> &bst) {
  int curr = (n+1)/2;
  recur(n,bst,curr,curr);
}
